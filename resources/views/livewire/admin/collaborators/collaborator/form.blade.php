    ...
    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
        ...
        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white uppercase">
                {{ data_get($collaborator, 'id') ? 'Editando' : 'Adicionar' }} Colaborador
            </h3>
        ...
        <form wire:submit.prevent="save" action="#" method="POST">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 sm:gap-3">
                ...
                <div class="sm:col-span-2 p-8">
                    <x-toggle left-label="Administrador?" wire:model.defer="is_admin" />
                </div>
            </div>

            <h2 class="p-2 mb-3 bg-gray-300 border border-t border-b rounded-t-md font-bold text-md uppercase">Documentos</h2>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 sm:gap-3">
                @foreach ($documentTypes as $documentType)
                    @switch($documentType->name)
                        ...
                        @case('rg')
                            <div class="sm:col-span-2">
                                <x-input x-mask="99.999.999-9" wire:model.lazy="document.{{ $documentType->name }}" label="{{ $documentType->label }}" placeholder="Digite o {{ $documentType->label }}" />
                            </div>
                            @break
                        @case('cnpj')
                            <div class="sm:col-span-2">
            ...
            <h2 class="p-2 mb-3 mt-6 bg-gray-300 border border-t border-b rounded-t-md font-bold text-md uppercase">Contatos</h2>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 sm:gap-3">
            ...
            <h2 class="p-2 mb-3 mt-6 bg-gray-300 border border-t border-b rounded-t-md font-bold text-md uppercase">Endereço</h2>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 sm:gap-3">
                <div class="sm:col-span-1 flex">
                    <x-input wire:model.debounce.500ms="zip_code" x-mask="99999-999" name="zip_code" label="CEP" placeholder="000000-000" autocomplete="off" />
                    <x-loading.timer />
                </div>
            ...
            @if (!data_get($collaborator, 'id'))
                ...
                <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 sm:gap-3 mt-4">
                    <div class="sm:col-span-2">
                        <x-input type="password" wire:model.lazy="password" label="Senha*" placeholder="********" />
                    </div>
                    <div class="sm:col-span-2">
                        <x-input type="password" wire:model.lazy="password_confirmation" label="Confirme a Senha*" placeholder="********" />
                    </div>
                </div>
            @endif
